...
export function OnboardingScreen() {
  ...
  const completeFunding = async () => {
    ...
    try {
      ...
      setFlow('depositing');
      const result = await fundVaultLifecycle({ vault: activeVault, mnemonic: wallet.identity.mnemonic, bitcoinRpcBaseUrl: wallet.baseUrl, daemonBaseUrl: wallet.daemonUrl, amountSats: 40_000n, feeRateSatVb: 2, onProgress: setFlow, onConfirmationPoll: setDepositConfirmations, onDepositBroadcast: deposit => { setDepositTxid(deposit.txid); setDepositConfirmations(0); localStorage.setItem(`ripcord:deposit:${activeVault.address}`, deposit.txid); } });
      setFlow('complete');
  ...
  const statusText = vaultReady ? 'Your vault is funded and registered. You can now receive and send VTXOs.' : flow === 'depositing' ? 'Funding is confirmed. We are now depositing 40,000 sats into your vault.' : flow === 'confirming-deposit' ? `Your vault deposit is broadcast with ${depositConfirmations} of 1 confirmations. You can safely leave and return later.` : flow === 'minting' ? 'Your deposit is confirmed. Creating your first spendable VTXO on Tachi.' : flow === 'registering' ? 'Your VTXO is ready. Registering the vault with live validators.' : flow === 'complete' ? 'Your wallet is funded, your first VTXO is created, and your vault is registered.' : pendingFaucetTxid ? 'Your faucet transaction is saved. We are checking it and will resume setup after it confirms.' : 'Fund this settlement address first, or continue if it already has confirmed L1 funds.';
  const fundingSteps = [
    { label: 'Faucet funds confirmed', done: flow !== 'ready' },
    { label: 'Vault deposit broadcast', done: ['confirming-deposit', 'minting', 'registering', 'complete'].includes(flow) },
