...
export function RipcordPanel() {
  const { vaults, identity } = useWallet(); const { readiness, assess } = useRipcord(); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const vault = vaults[0];
  ...
  return <section id="ripcord" className="ripcord-panel" aria-labelledby="ripcord-title">
    ...
    {readiness?.dryRun && <div className="dry-run"><strong>Test-pull verified</strong><dl><div><dt>txid</dt><dd>{truncate(readiness.dryRun.txid, 12, 10)}</dd></div><div><dt>vsize</dt><dd>{readiness.dryRun.vsize} vB</dd></div><div><dt>nSequence</dt><dd>{readiness.dryRun.sequence}</dd></div></dl><p>Nothing was broadcast. No sats moved.</p></div>}
    {error && <p className="inline-error" role="alert">{error}</p>}
    {vault && <TapscriptInspector vault={vault} />}
    <div className="ripcord-actions"><button className="test-pull" disabled={!vault || !identity || busy} onClick={() => void run()}>{busy ? 'Building exit…' : readiness ? 'Run test-pull again' : 'Run test-pull'}</button><HoldToConfirmButton disabled={status !== 'live'} onConfirm={() => setError('Broadcast is connected in Phase 12 after destination review.')} /></div>
  </section>;
}
