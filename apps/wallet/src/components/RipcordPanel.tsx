...
export function RipcordPanel() {
  const { activeVault: vault, identity, setExitReadiness } = useWallet(); const { readiness, refreshMaturity, assess } = useRipcord(); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  ...
  return <section id="ripcord" className="ripcord-panel" aria-labelledby="ripcord-title">
    ...
    <div className="ripcord-actions"><button className="test-pull" disabled={!vault || !identity || busy} onClick={() => void run()}>{busy ? 'Building exit…' : readiness?.dryRun ? 'Run test-pull again' : 'Run test-pull'}</button><HoldToConfirmButton disabled={status !== 'live' || !readiness?.dryRun} onConfirm={() => setError('Broadcast is connected in Phase 12 after destination review.')} /></div>
  </section>;
}
