...
if grep -rnE "addressTransactions|listTransactions|finalizeVtxoPsbt" "${DIRS[@]}" 2>/dev/null | grep -v "test/" | grep -v "dist/"; then
...
if grep -rnE "$MOCK_PATTERN" "${DIRS[@]}" --include='*.ts' --include='*.tsx' 2>/dev/null | grep -v "dist/"; then
...
if grep -rnE "\.skip\(" "${DIRS[@]}" --include='*.ts' --include='*.tsx' 2>/dev/null | grep -v "dist/"; then
